...
pub fn parse(
        ...
        .filter_map(|field| {
            let struct_visitor = |struct_name: syn::Path| {
                ...
                Some(quote! {
                  ...
                  impl<'de> crate::xml::Visitor<'de> for #visitor_label {
                    ...
                    fn visit_str(
                      ...
                      let value = crate::xml::__xml::escape::escape_str_pcdata(v);
                      let content = format!("{}{}{}", #xml_opening, value, #xml_closing);
                      crate::xml::de::from_str(&content)
                    }
                  }
                })
            };
      ...
      let visit_struct = |struct_name: syn::Path, action: TokenStream| {
        Some(quote! {
          (#namespace, #label_name) => {
            ...
            if let Ok(crate::xml::__xml::reader::XmlEvent::StartElement { .. }) = reader.peek() {
              // If substruct's start element found then deserialize substruct
              let value = <#struct_name as crate::xml::XmlDeserialize>::deserialize(reader)?;
              #value_label #action;
              // read EndElement
        ...
        .map(|field| {
            ...
            match field.get_type() {
                Field::FieldStruct { .. } => quote! {
                  #value_label = Some(crate::xml::de::from_str(&unused_xml_elements)?);
                },
                Field::FieldOption { data_type } => match *data_type {
                    Field::FieldStruct { .. } => quote! {
                      #value_label = crate::xml::de::from_str(&unused_xml_elements).ok();
                    },
                    field_type => {
    ...
    quote! {
      impl #impl_generics crate::xml::XmlDeserialize for #name #ty_generics #where_clause {
        #[allow(unused_variables)]
        fn deserialize<R: ::std::io::Read>(
          reader: &mut crate::xml::de::Deserializer<R>,
        ) -> ::std::result::Result<Self, ::std::string::String> {
          let (named_element, struct_namespace) =
            if let crate::xml::__xml::reader::XmlEvent::StartElement { name, .. } = reader.peek()?.to_owned() {
              (name.local_name.to_owned(), name.namespace.clone())
            } else {
              (::std::string::String::from(#root), ::std::option::Option::None)
            };
          let start_depth = reader.depth();
          crate::xml::__derive_debug!("Struct {} @ {}: start to parse {:?}", stringify!(#name), start_depth,
                 named_element);
          ...
          loop {
            ...
            crate::xml::__derive_trace!(
              "Struct {} @ {}: matching {:?}",
              stringify!(#name), start_depth, event,
            );
            match event {
              crate::xml::__xml::reader::XmlEvent::StartElement{ref name, ref attributes, ..} => {
              ...
              crate::xml::__xml::reader::XmlEvent::EndElement { ref name } => {
              ...
              crate::xml::__xml::reader::XmlEvent::EndDocument => {
                if #flatten {
                  break;
                }
              }
              crate::xml::__xml::reader::XmlEvent::Characters(ref text_content) => {
                #set_text
                let event = reader.next_event()?;
                #write_unused
              }
          ...
          crate::xml::__derive_debug!("Struct {} @ {}: success", stringify!(#name), start_depth);
    ...
    Some(quote! {
      (#namespace, #label_name) => {
        ...
        let result = reader.read_inner_value::<#field_type, _>(|reader| {
          if let ::std::result::Result::Ok(crate::xml::__xml::reader::XmlEvent::Characters(s)) = reader.peek() {
...
fn build_code_for_unused_xml_events(
    ...
    (
        Some(quote! {
          let mut buf = ::std::vec![];
          let mut writer = ::std::option::Option::Some(crate::xml::__xml::writer::EventWriter::new(&mut buf));
        }),
        Some(quote! {
