    ...
    quote! {
      impl #impl_generics crate::xml::XmlDeserialize for #name #ty_generics #where_clause {
        ...
        fn deserialize<R: ::std::io::Read>(
          ...
          crate::xml::__derive_debug!("Enum {} @ {}: start to parse {:?}", stringify!(#name), start_depth, named_element);
          ...
          loop {
            ...
            crate::xml::__derive_trace!("Enum {} @ {}: matching {:?}", stringify!(#name), start_depth, event);
            match event {
              crate::xml::__xml::reader::XmlEvent::StartElement { ref name, ref attributes, .. } => {
                ...
                if let crate::xml::__xml::reader::XmlEvent::Characters(content) = reader.peek()?.to_owned() {
          ...
          crate::xml::__derive_debug!("Enum {} @ {}: success", stringify!(#name), start_depth);
...
fn parse_variant(variant: &syn::Variant, name: &Ident) -> Option<TokenStream> {
    let xml_element_name = XmlAttribute::from(&variant.attrs).xml_element_name(&variant.ident);
...
fn build_unnamed_visitor_calls(
    ...
    .filter_map(|(idx, field)| {
      ...
      let call_simple_type_visitor = |simple_type: Field, action| {
        ...
        Some(quote! {
          ...
          let result = reader.read_inner_value::<#field_type, _>(|reader| {
            if let crate::xml::__xml::reader::XmlEvent::EndElement { .. } = *reader.peek()? {
              return visitor.#visitor("");
            }

            if let ::std::result::Result::Ok(crate::xml::__xml::reader::XmlEvent::Characters(s))
              = reader.next_event()
            {
              visitor.#visitor(&s)
            } else {
      ...
      let call_struct_visitor = |struct_name, action| {
        Some(quote! {
          match <#struct_name as crate::xml::XmlDeserialize>::deserialize(reader) {
            Ok(value) => {
              #action;
              let _root = reader.next_event();
            },
