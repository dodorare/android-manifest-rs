...
impl TryFrom<&Attribute> for XmlAttribute {
    type Error = String;
        ...
        if attr.path().is_ident("xml") {
            let attributes = match &attr.meta {
                ...
                Meta::List(list) => {
                    ...
                    match from_tokenstream::<XmlAttribute>(&tokens) {
                        Ok(attribute) => attribute,
                        Err(error) => {
                            panic!("XML mapping derive error: {}", error);
                        }
                    }
                }
                Meta::NameValue(_) => {
...
impl From<&Vec<Attribute>> for XmlAttribute {
    fn from(attributes: &Vec<Attribute>) -> Self {
        attributes
            .iter()
            .find_map(|attribute| XmlAttribute::try_from(attribute).ok())
            .unwrap_or_default()
    }
}
