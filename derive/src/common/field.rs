use crate::common::attribute::XmlAttribute;
...
impl XmlField {
    pub fn new(syn_field: syn::Field) -> Self {
        let attributes = XmlAttribute::from(&syn_field.attrs);
    ...
    pub fn renamed_label(&self, root_attributes: &XmlAttribute) -> String {
    ...
    pub fn prefix_namespace(&self, root_attributes: &XmlAttribute) -> String {
        root_attributes
            .namespaces
            .iter()
    ...
    pub fn ser_wrap_default_attribute(
        ...
        let xml_inner_definition = builder
            .map(|builder| quote!(let xml_inner = #builder;))
            .unwrap_or_default();
...
impl From<&syn::Field> for Field {
    fn from(field: &syn::Field) -> Self {
        ...
        match ty {
            Path(path) => Field::from(&path.path),
    ...
    fn from(path_segment: &syn::PathSegment) -> Self {
        if let syn::PathArguments::AngleBracketed(ref args) = path_segment.arguments {
            match args.args.first() {
                Some(syn::GenericArgument::Type(Path(path))) => {
                    return Field::from(&path.path);
                }
                Some(syn::GenericArgument::Type(syn::Type::Group(syn::TypeGroup {
                    elem, ..
                }))) => {
                    if let Path(group) = elem.as_ref() {
