...
pub fn serialize(
    ...
    .map(|variant| -> TokenStream {
      let add_tag = if let Some(tag) = &root_attributes.tag {
        let attrs = crate::common::XmlAttribute::from(&variant.attrs);
        ...
        quote! {
          match self {
            #name::#label { .. } => {
              let tag = crate::xml::__xml::name::OwnedName::local(#tag);
              child_attributes.push(crate::xml::__xml::attribute::OwnedAttribute::new(tag, #element_name));
            }
            _ => {}
          }
        }
...
fn inner_enum_inspector(
    ...
    .iter()
    .map(|variant| {
      let variant_attrs = XmlAttribute::from(&variant.attrs);
      ...
      match variant.fields {
        Fields::Unit => {
          if let Some(_tag) = &root_attributes.tag {
            ...
            quote! {
              #name::#label => {
                let data_event = crate::xml::__xml::writer::XmlEvent::characters(#label_name);
          ...
          let enum_fields: TokenStream = fields
            ...
            .filter_map(|field| {
              ...
              if field.is_text_content() {
                return Some(quote!(
                  let data_event = crate::xml::__xml::writer::XmlEvent::characters(&self.#field_label);
                  writer.write(data_event).map_err(|e| e.to_string())?;
                ));
              }
              ...
              match field.get_type() {
                ...
                | Field::FieldF64 => Some({
                  quote! {
                    match self {
                      #name::#label { #field_label, .. } => {
                        let struct_start_event =
                          crate::xml::__xml::writer::XmlEvent::start_element(#field_label_name);
                        writer.write(struct_start_event).map_err(|e| e.to_string())?;

                        let string_value = #field_label.to_string();
                        let data_event = crate::xml::__xml::writer::XmlEvent::characters(&string_value);
                        writer.write(data_event).map_err(|e| e.to_string())?;
                ...
                Field::FieldStruct { .. } => Some(quote! {
                  match self {
                    #name::#label { #field_label, .. } => {
                      ...
                      crate::xml::XmlSerialize::serialize(#field_label, writer)?;
                    },
                    _ => {}
                  }
                }),
          ...
          let enum_fields: TokenStream = fields
            ...
            .map(|field| {
              let write_element = |action: &TokenStream| {
                quote! {
                  let struct_start_event = crate::xml::__xml::writer::XmlEvent::start_element(#label_name);
                  writer.write(struct_start_event).map_err(|e| e.to_string())?;
              ...
              let write_string_chars = quote! {
                let data_event = crate::xml::__xml::writer::XmlEvent::characters(item);
                writer.write(data_event).map_err(|e| e.to_string())?;
              };
              ...
              match field.get_type() {
                ...
                Field::FieldStruct { .. } => {
                  if variant_attrs.flatten || field.is_flatten() {
                     match_field(&quote!{ crate::xml::XmlSerialize::serialize(item, writer)?})
                   } else {
