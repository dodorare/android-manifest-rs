...
fn enclose_xml_event(label_name: String, xml_format: TokenStream) -> TokenStream {
    quote! {
      let start_event = crate::xml::__xml::writer::XmlEvent::start_element(#label_name);
      writer.write(start_event).map_err(|e| e.to_string())?;

      let xml_value = #xml_format;
      let data_event = crate::xml::__xml::writer::XmlEvent::characters(&xml_value);
...
pub fn condition_generator(label: &Option<Ident>, field: &XmlField) -> TokenStream {
