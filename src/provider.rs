...
/// element in the manifest file; otherwise, the system is unaware of them and
/// doesn't run them. You only declare content providers that are part of your
/// application. Content providers in other applications that you use in your
...
/// ```
///
/// The `content:` `scheme` identifies the URI as a content URI pointing to an
/// Android content provider. The authority
/// `com.example.project.healthcareprovider` identifies the provider itself; the
/// Android system looks up the authority in its list of known providers and
/// their authorities. The substring `nurses/rn` is a `path`, which the content
/// provider can use to identify subsets of the provider data. Notice that when
...
pub struct Provider {
    /// A list of one or more URI authorities that identify data offered by the
    /// content provider. Multiple authorities are listed by separating
    /// their names with a semicolon. To avoid conflicts, authority names
    /// should use a Java-style naming convention (such as
    ...
    /// (as they both are by default) for the service to be enabled. If either
    /// is `"false"`, the service is disabled; it cannot be instantiated.
    ...
    /// permissions specified for the provider. `false:` The provider is not
    /// available to other applications. Set android:exported="false" to limit
    /// access to the provider to your applications. Only applications that
    /// have the same user ID (UID) as the provider, or applications that have
    /// been temporarily granted access to the provider through the
    /// android:grantUriPermissions element, have access to it. Because this
    /// attribute was introduced in API level 17, all devices running API level
    /// 16 and lower behave as though this attribute is set `"true"`. If you
    ...
    /// temporarily overcoming the restriction imposed by the `readPermission`,
    /// `writePermission`, `permission`, and `exported` attributes — `"true"` if
    /// permission can be granted, and `"false"` if not. If `"true"`, permission
    /// can be granted to any of the content provider's data. If `"false"`,
    /// permission can be granted only to the data subsets listed in
    /// `<grant-uri-permission>` subelements, if any. The default value is
    /// `"false"`. Granting permission is a way of giving an application
    /// component one-time access to data protected by a permission. For
    /// example, when an e-mail message contains an attachment, the mail
    /// application may call upon the appropriate viewer to open it, even
    /// though the viewer doesn't have general permission to look at all the
    /// content provider's data. In such cases, permission is granted by
    /// FLAG_GRANT_READ_URI_PERMISSION and FLAG_GRANT_WRITE_URI_PERMISSION flags
    /// in the Intent object that activates the component. For example, the
    ...
    /// to the URI in the Intent. If you enable this feature, either by
    /// setting this attribute to `"true"` or by defining
    /// `<grant-uri-permission>` subelements, you must call
    /// `Context.revokeUriPermission()` when a covered URI is deleted from the
    /// provider. See also the `<grant-uri-permission>` element.
    ...
    /// If it is not set, the icon specified for the application as a whole is
    ...
    /// The order in which the content provider should be instantiated, relative
    /// to other content providers hosted by the same process. When there
    /// are dependencies among content providers, setting this attribute for
    /// each of them ensures that they are created in the order required by
    /// those dependencies. The value is a simple integer, with higher
    /// numbers being initialized first.
    #[serde(rename = "android:initOrder")]
    pub init_order: Option<i32>,
    /// A user-readable label for the content provided. If this attribute is not
    /// set, the label set for the application as a whole is used instead (see
    /// the `<application>` element's `label` attribute). The label should
    /// be set as a reference to a string resource, so that it can be localized
    ...
    /// If true, each of the app's processes has its own content provider
    /// object. If false, the app's processes share only one content provider
    /// object. The default value is false. Setting this flag to true may
    /// improve performance by reducing the overhead of interprocess
    /// communication, but it also increases the memory footprint of each
    /// process.
    #[serde(rename = "android:multiprocess")]
    pub multiprocess: Option<bool>,
    /// The name of the class that implements the content provider, a subclass
    /// of `ContentProvider`. This should be a fully qualified class name (such
    ...
    /// access for modifying the provider's data. For more information on
    /// permissions, see the `Permissions` section in the introduction and a
    /// separate document, `Security and Permissions`.
    ...
    /// different default for all components. But each component can
    /// override the default with its own `process` attribute, allowing you to
    /// spread your application across multiple processes. If the name
    /// assigned to this attribute begins with a colon (':'), a new process,
    /// private to the application, is created when it's needed and the activity
    /// runs in that process. If the process name begins with a lowercase
    ...
    /// If the provider sets `android:grantUriPermissions` to true, or if a
    /// given client satisfies the conditions of a `<grant-uri-permission>`
    /// subelement, the client can gain temporary read access to the content
    /// provider's data. See also the `permission` and `writePermission`
    /// attributes.
    #[serde(rename = "android:readPermission")]
    pub read_permission: Option<String>,
    /// Whether or not the data under the content provider's control is to be
    /// synchronized with data on a server — `"true"` if it is to be
    /// synchronized, and `"false"` if not.
    #[serde(rename = "android:syncable")]
    pub syncable: Option<bool>,
    /// A permission that clients must have to make changes to the data
    /// controlled by the content provider. If the provider sets
    /// `android:grantUriPermission`s to true, or if a given client satisfies
    /// the conditions of a `<grant-uri-permission>` subelement the client
