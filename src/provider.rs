...
/// content provider can use to identify subsets of the provider data. Notice that when
...
pub struct Provider {
    ...
    /// be, and `"false"` if not. The default value is `"true"`. The [`<application>`]
    /// element has its own [`enabled`] attribute that applies to all application
    /// components, including services. The [`<application>`] and <service> attributes
    /// must both be `"true"` (as they both are by default) for the service to be
    ...
    /// The default value is `"false"`. Granting permission is a way of giving an
    /// application component one-time access to data protected by a permission. For
    ...
    /// share only one content provider object. The default value is false. Setting
    /// this flag to true may improve performance by reducing the overhead of
    ...
    /// processes. If the name assigned to this attribute begins with a colon (':'), a
    ...
    /// A permission that clients must have to query the content provider. If the provider
    /// sets `android:grantUriPermissions` to true, or if a given client satisfies the
    /// conditions of a `<grant-uri-permission>` subelement, the client can gain
    /// temporary read access to the content provider's data. See also the
    ...
    /// content provider. If the provider sets `android:grantUriPermission`s to true,
    /// or if a given client satisfies the conditions of a `<grant-uri-permission>`
    /// subelement the client can gain temporary write access to modify the content
    /// provider's data. See also the `permission` and `readPermission` attributes.
