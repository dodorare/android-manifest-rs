...
/// For example, if the base name is com.example.project.taxes.
///
...
/// See the `<permission>` element for information on declaring permissions.
...
pub struct PermissionTree {
    ...
    /// the label can be directly set as a raw string for quick and dirty programming.
    /// However, when the application is ready to be published, 
    ...
    /// The name must have more than two period-separated segments in its path — for example,
    /// com.example.base is OK, but com.example is not.
