...
/// Declares the base name for a tree of permissions. The application takes ownership of all names within the tree. It can dynamically add new permissions to the tree by calling PackageManager.addPermission()
/// Names within the tree are separated by periods ('.'). For example, if the base name is com.example.project.taxes, permissions like the following might be added:
...
pub struct PermissionTree {
    ...
    /// A user-readable name for the group. As a convenience, the label can be directly set as a raw string for quick and dirty programming.
