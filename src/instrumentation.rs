...
pub struct Instrumentation {
    ...
    /// Whether or not the Instrumentation object will turn profiling on and off — `"true"` if it determines when profiling starts and stops, and `"false"` if profiling continues the entire time it is running.
    /// A value of `"true"` enables the object to target profiling at a specific set of operations. The default value is `"false"`.
    ...
    /// The name of the Instrumentation subclass. This should be a fully qualified class name (such as, `"com.example.project.StringInstrumentation"`).
    ...
    /// The processes that the `Instrumentation` object will run against. A comma-separated list indicates that the instrumentation will run against those specific processes.
