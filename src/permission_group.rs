...
///
/// Individual permission join the group through the permissionGroup attribute
/// of the `<permission>` element. Members of a group are presented together in
/// the user interface. Note that this element does not declare a permission
/// itself, only a category in which permissions can be placed.
/// See the `<permission>` element for element for information on declaring
/// permissions and assigning them to groups.
///
/// ## Contained in:
...
pub struct PermissionGroup {
    /// User-readable text that describes the group. The text should be longer
    /// and more explanatory than the label. This attribute must be set as a
    /// reference to a string resource. Unlike the label attribute, it cannot be
    /// a raw string.
    ...
    /// as a reference to a string resource, so that it can be localized
