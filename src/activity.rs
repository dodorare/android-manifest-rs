...
pub struct Activity {
    ...
    /// other applications — "true" if it can be, and "false" if not.
    ...
    /// the system tries to match an implicit intent. This attribute is not the
    /// only way to limit an activity's exposure to other applications.
    /// You can also use a permission to limit the external entities that can
    /// invoke the activity (see the permission attribute).
    ...
    /// hardware-accelerated renderer is enabled, most operations in Canvas,
    ...
    /// `android:label` attribute). This attribute must be set as a
    /// reference to a drawable resource containing the image definition. If it
    /// is not set, the icon specified for the application as a whole is
    /// used instead (see the `<application>` element's `icon` attribute). The
    /// activity's icon — whether set here or by the `<application>` element —
    /// is also the default icon for all the activity's intent filters (see
    /// the `<intent-filter>` element's `icon` attribute).
    ...
    /// along with the activity icon. If this attribute is not set, the
    /// label set for the application as a whole is used instead (see the
    /// `<application>` element's `label` attribute). The activity's label —
    /// whether set here or by the `<application>` element — is also the default
    /// label for all the activity's intent filters (see the `<intent-filter>`
    /// element's `label` attribute). The label should be set as a reference
    /// to a string resource, so that it can be localized like other strings in
    /// the user interface. However, as a convenience while you're
    ...
    /// chosen — see the `taskAffinity` attribute). In contrast,
    /// `"singleTask"` and `"singleInstance"` activities can only begin a task.
    /// They are always at the root of the activity stack. Moreover, the
    /// device can hold only one instance of the activity at a time — only one
    /// such task. The `"standard"` and `"singleTop"` modes differ from each
    /// other in just one respect: Every time there's a new intent for a
    /// `"standard"` activity, a new instance of the class is created to respond
    /// to that intent. Each instance handles a single intent. Similarly, a
    /// new instance of a `"singleTop"` activity may also be created to handle a
    /// new intent. However, if the target task already has an existing
    ...
    /// stack. Similarly, if you navigate up to an activity on the current
    /// stack, the behavior is determined by the parent activity's launch mode.
    /// If the parent activity has launch mode `singleTop` (or the up intent
    /// contains `FLAG_ACTIVITY_CLEAR_TOP`), the parent is brought to the
    /// top of the stack, and its state is preserved. The navigation intent is
    /// received by the parent activity's `onNewIntent()` method. If the parent
    /// activity has launch mode standard (and the up intent does not
    ...
    /// Determines how the system presents this activity when the device is
    /// running in lock task mode. Android can run tasks in an immersive,
    /// kiosk-like fashion called lock task mode. When the system runs in lock
    /// task mode, device users typically can’t see notifications, access
    /// non-allowlisted apps, or return to the home screen (unless the Home
    /// app is allowlisted). Only apps that have been allowlisted by a device
    /// policy controller (DPC) can run when the system is in lock task mode.
    ...
    /// The maximum number of tasks rooted at this activity in the overview
    /// screen. When this number of entries is reached, the system removes the
    /// least-recently used instance from the overview screen. Valid values
    /// are 1 through 50 (25 on low memory devices); zero is invalid. This must
    /// be an integer value, such as 50. The default value is 16.
    #[serde(rename = "android:maxRecents")]
    pub max_recents: Option<i32>,
    /// The maximum aspect ratio the activity supports. If the app runs on a
    /// device with a wider aspect ratio, the system automatically letterboxes
    /// the app, leaving portions of the screen unused so the app can run at its
    /// specified maximum aspect ratio. Maximum aspect ratio is expressed as the
    /// decimal form of the quotient of the device's longer dimension divided by
    /// its shorter dimension. For example, if the maximum aspect ratio is
    /// 7:3, set the value of this attribute to 2.33. On non-wearable devices,
    ...
    /// name specified in the `<manifest>` element. Once you publish your
    /// application, you should not change this name (unless you've set
    /// `android:exported="false"`). There is no default. The name must be
    /// specified.
    ...
    /// result from this activity. This attribute was introduced in API
    /// Level 3.
    ...
    /// match the class name given to the corresponding `<activity>` element's
    /// `android:name` attribute. The system reads this attribute to
    /// determine which activity should be started when the user presses the Up
    /// button in the action bar. The system can also use this information to
    /// synthesize a back stack of activities with `TaskStackBuilder`.
    ...
    /// Defines how an instance of an activity is preserved within a containing
    /// task across device restarts. If the root activity of a task sets
    /// this attribute's value to `persistRootOnly`, then only the root activity
    /// is preserved. Otherwise, the activities that are higher up the
    /// task's back stack are examined; any of these activities that set this
    /// attribute's value to `persistAcrossReboots` are preserved
    ...
    /// the application and you do not need to use this attribute. But if
    /// necessary, you can override the default process name with this
    /// attribute, allowing you to spread your app components across multiple
    /// processes. If the name assigned to this attribute begins with a
    /// colon (':'), a new process, private to the application, is created when
    /// it's needed and the activity runs in that process. If the process name
    ...
    /// allows components in different applications to share a process, reducing
    /// resource usage. The `<application>` element's `process` attribute
    /// can set a different default process name for all components.
    ...
    /// activity takes over the full screen. If your app targets API level
    /// 24 or higher, but you do not specify a value for this attribute, the
    /// attribute's value defaults to true. This attribute was added in API
    /// level 24.
    #[serde(rename = "android:resizeableActivity")]
    pub resizeable_activity: Option<bool>,
    /// The orientation of the activity's display on the device. The system
    /// ignores this attribute if the activity is running in multi-window mode.
    ...
    /// value. This attribute was added in API level 23.
    ...
    /// affinity conceptually belong to the same task (to the same
    /// `"application"` from the user's perspective). The affinity of a task
    /// is determined by the affinity of its root activity. The affinity
    ...
    /// activities defined in different applications within the same task.
    /// To specify that the activity does not have an affinity for any task, set
    /// it to an empty string. If this attribute is not set, the activity
    /// inherits the affinity set for the application (see the `<application>`
    /// element's taskAffinity attribute). The name of the default affinity
    /// for an application is the package name set by the `<manifest>` element.
    ...
    /// activity. This automatically sets the activity's context to use this
    /// theme (see `setTheme()` and may also cause `"starting"` animations
    /// prior to the activity being launched (to better match what the activity
    /// actually looks like). If this attribute is not set, the activity
    /// inherits the theme set for the application as a whole — from the
    /// `<application>` element's theme attribute. If that attribute is also
    /// not set, the default system theme is used. For more information, see the
    /// Styles and Themes developer guide.
...
pub enum DocumentLaunchMode {
    /// The system searches for a task whose base intent's ComponentName and
    /// data URI match those of the launching intent. If the system finds
    /// such a task, the system clears the task, and restarts with the root
    /// activity receiving a call to `onNewIntent(android.content.Intent)`.
    /// If the system does not find such a task, the system creates a new task.
    IntoExisting,
    /// The activity creates a new task for the document, even if the document
    /// is already opened. This is the same as setting both the
    /// `FLAG_ACTIVITY_NEW_DOCUMENT` and `FLAG_ACTIVITY_MULTIPLE_TASK` flags
    Always,
    /// The activity does not create a new task for the activity. This is the
    /// default value, which creates a new task only when FLAG_ACTIVITY_NEW_TASK
    ...
    /// contains FLAG_ACTIVITY_NEW_DOCUMENT. Setting this overrides the
    /// behavior of the FLAG_ACTIVITY_NEW_DOCUMENT and
    /// FLAG_ACTIVITY_MULTIPLE_TASK flags, if either of these are set in the
    /// activity, and the overview screen displays a single task for the
    /// app, which resumes from whatever activity the user last invoked.
    Never,
}
...
pub enum LaunchMode {
    ...
    /// target task and routes the intent to it.
    Standard,
    ...
    /// activity.
    SingleTop,
    /// The system creates the activity at the root of a new task and routes the
    /// intent to it. However, if an instance of the activity already exists,
    ...
    /// activities into the task holding the instance. The activity is
...
pub enum LockTaskMode {
    /// Default value. This is the default value. Tasks don't launch into lock
    ...
    /// locked task. If the DPC does not authorize this package then this mode
...
pub enum PersistableMode {
    ///	Default value. When the system restarts, the activity task is preserved,
...
pub enum ScreenOrientation {
    ...
    /// based on the device sensor. The sensor is used even if the user has
    /// locked sensor-based rotation. Added in API level 9.
    SensorLandscape,
    /// Portrait orientation, but can be either normal or reverse portrait based
    /// on the device sensor. The sensor is used even if the user has locked
    /// sensor-based rotation. Added in API level 9.
    SensorPortrait,
    /// Landscape orientation, but can be either normal or reverse landscape
    /// based on the device sensor and the user's preference. Added in API level
    /// 18.
    UserLandscape,
    /// Portrait orientation, but can be either normal or reverse portrait based
    /// on the device sensor and the user's preference. Added in API level 18.
    UserPortrait,
    ...
    /// To allow all four orientations, use "fullSensor" The sensor is used even
    /// if the user locked sensor-based rotation.
    Sensor,
    ...
    /// reverse portrait or reverse landscape, but this enables those). Added in
    /// API level 9.
    FullSensor,
    ...
    /// rotate based on how the user moves the device.
    Nosensor,
    /// The user's current preferred orientation.
    User,
    /// If the user has locked sensor-based rotation, this behaves the same as
    /// user, otherwise it behaves the same as fullSensor and allows any of the
    /// 4 possible screen orientations. Added in API level 18.
    FullUser,
    /// Locks the orientation to its current rotation, whatever that is. Added
    /// in API level 18.
    Locked,
}
...
pub enum WindowSoftInputMode {
    ...
    /// smaller area. This is the default setting for the behavior of the
    /// main window.
    AdjustUnspecified,
