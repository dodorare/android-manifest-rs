...
/// or other applications. See the Permissions section in the introduction,
///
/// ## Contained in:
...
pub struct Permission {
    ...
    /// This attribute must be set as a reference to a string resource; unlike
    /// the `label` attribute, it cannot be a raw string.
    ...
    /// As a convenience, the label can be directly set as a raw string while
    /// you're developing the application. However, when the application is
    /// ready to be published, it should be set as a reference to a string
    /// resource, so that it can be localized like other strings in the user
    /// interface.
    #[serde(rename = "android:label")]
    pub label: Option<StringResourceOrString>,
    /// The name of the permission. This is the name that will be used in code
    /// to refer to the permission — for example, in a <uses-permission>
    /// element and the permission attributes of application components.
    /// ## Note
    ...
    /// Assigns this permission to a group. The value of this attribute is the
    /// name of the group, which must be declared with the
    /// `<permission-group>` element in this or another application. If this
    ...
    /// the procedure the system should follow when determining whether or
    /// not to grant the permission to an application requesting it.
    /// Each protection level consists of a base permission type and zero or
    /// more flags. For example, the `"dangerous"` protection level has no
    /// flags. In contrast, the protection level `"signature|privileged"` is
    /// a combination of the `"signature"` base permission type and the
    /// `"privileged"` flag.
...
pub enum ProtectionLevel {
    ...
    /// to private user data or control over the device that can negatively
    /// impact the user. Because this type of permission introduces
    ...
    /// confirmation before proceeding, or some other approach may
    ...
    /// that are in a dedicated folder on the Android system image
    /// or that are signed with the same certificate as the application that
    /// declared the permission. Avoid using this option, as the signature
    /// protection level should be sufficient for most needs and works
