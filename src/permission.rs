...
///
/// That can be used to limit access to specific components or features of this or other applications.
...
pub struct Permission {
    ...
    /// It may be displayed to explain the permission to the user — for example,
    ///  when the user is asked whether to grant the permission to another application.
    ...
    /// ## Note
    /// The system does not allow multiple packages to declare a permission with the same name, unless all the
...
pub enum ProtectionLevel {
    ...
    /// Avoid using this option, as the signature protection level should be sufficient
    /// for most needs and works regardless of exactly where apps are installed.
