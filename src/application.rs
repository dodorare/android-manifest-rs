...
pub struct Application {
    ...
    /// Single-package restore operations will never cause the application to be shut down. Full-system restore operations typically only occur once, when the phone is first set up.
    ...
    /// It only applies to the first application loaded into a process; if you're using a shared user ID to allow multiple applications to
    /// use a process, they all must use this option consistently or they will have unpredictable results.
    ...
    /// By default, Android creates a process for an application when the first of its components needs to run. All components then run in that process.
    /// The name of the default process matches the package name set by the `<manifest>` element.
    ...
    /// Specifies the account type required by the application in order to function. If your app requires an `Account`,
    /// the value for this attribute must correspond to the account authenticator type used by your app (as defined by `AuthenticatorDescription`), such as "com.google".
    /// The default value is null and indicates that the application can work without any accounts.
    /// Because restricted profiles currently cannot add accounts, specifying this attribute makes your app unavailable from a restricted profile unless you also
    ...
    /// if set to false or if targetSdkVersion is set to 16 or lower, the RTL APIs will be ignored or will have no effect and your app will behave the same regardless of the
    /// layout direction associated to the user's Locale choice (your layouts will always be left-to-right). The default value of this attribute is false
    ...
    /// Indicates whether this application is only for testing purposes. For example, it may expose functionality or data outside of itself that would
    /// cause a security hole, but is useful for testing. This kind of APK can be installed only through `adb`—you cannot publish it to Google Play.
    ...
    /// Indicates whether the app intends to use cleartext network traffic, such as cleartext HTTP. The default value for apps that target API level 27 or lower is `"true"`.
    ...
    /// Third-party libraries are strongly encouraged to honor this setting as well. The key reason for avoiding cleartext traffic is the lack of confidentiality, authenticity, and protections against tampering;
    /// a network attacker can eavesdrop on transmitted data and also modify it without being detected.
...
pub enum UiOptions {
    ...
    /// Instead of a small number of action items appearing in the app bar at the top of the screen, the app bar is split into the top navigation
    /// section and the bottom bar for action items. This ensures a reasonable amount of space is made available not only for the action items, but also for navigation and title
    /// elements at the top. Menu items are not split across the two bars; they always appear together.
