...
pub struct UsesNativeLibrary {
    ...
    pub name: String,
    /// Boolean value that indicates whether the application requires the library specified
    /// by android:name: 
    ///
    ...
    ///
    /// * `"false"`: The application can use the library if present, but is designed to function
    /// without it if necessary. The system will allow the application to be installed, even if
    /// the library is not present. If you use `"false"`, you are responsible for gracefully handling
