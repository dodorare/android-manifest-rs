...
pub struct UsesSdk {
    ...
    /// version. The application is still able to run on older versions
    /// (down to minSdkVersion). As Android evolves with each new version,
    /// some behaviors and even appearances might change. However, if the API
    /// level of the platform is higher than the version declared by your app's
    ...
    /// designed to run. In Android 1.5, 1.6, 2.0, and 2.0.1, the system checks
    /// the value of this attribute when installing an application
    /// and when re-validating the application after a system update. In either
    /// case, if the application's maxSdkVersion attribute is lower than the API
    /// Level used by the system itself, then the system will not allow
    /// the application to be installed. In the case of re-validation after
    /// system update, this effectively removes your application from the
    /// device. To illustrate how this attribute can affect your application
