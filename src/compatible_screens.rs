...
/// compatible. Only one instance of the `<compatible-screens>` element is
...
/// with which the application is compatible. The Android system does not read
/// the `<compatible-screens>` manifest element (neither at install-time nor at
/// runtime). This element is informational only and may be used by external
/// services (such as Google Play) to better understand the application's
...
/// screen with which the application is not compatible.
///
